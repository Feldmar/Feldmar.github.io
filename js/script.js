/* Задания на урок:

1) Удалить все рекламные блоки со страницы (правая часть сайта)

2) Изменить жанр фильма, поменять "комедия" на "драма"

3) Изменить задний фон постера с фильмом на изображение "bg.jpg". Оно лежит в папке img.
Реализовать только при помощи JS

4) Список фильмов на странице сформировать на основании данных из этого JS файла.
Отсортировать их по алфавиту 

5) Добавить нумерацию выведенных фильмов */

'use strict';

const movieDB = {
    movies: [
        "Я легенда",
        "Логан",
        "Лига справедливости",
        "Ла-ла лэнд",
        "Одержимость",
        "Форд против ферари"
    ]
};
movieDB.movies = movieDB.movies.sort((film1, film2) => film1.localeCompare(film2));

const promo__adv = document.querySelector('.promo__adv');
// promo__adv.style = 'display: none'
// promo__adv.remove()
promo__adv.textContent = ''

const promo__genre = document.querySelector('.promo__genre');
promo__genre.textContent = 'драма';

const bg = document.querySelector('.promo__bg');
bg.style = 'background-image: url(img/bg.jpg);';

const list = document.querySelector('.promo__interactive-list');

render();


function render() {
    list.textContent = '';
    movieDB.movies.forEach((film, index) => {
        list.innerHTML += `
            <li class="promo__interactive-item">№${index + 1}: ${film}
                <div class="delete" data-title='${film}' data-index='${index}'></div>
            </li>
        `;
    })
}

list.addEventListener('click', (event) => { //делегирование событий
    const target = event.target;
    if (target.classList.contains('delete')) {
        movieDB.movies.splice(target.dataset.index, 1);
        render();
    }
})

// const items = document.querySelectorAll('.promo__interactive-item');

// items.forEach((item) => {
//     item.addEventListener('click', (event) => {
//         console.log(event);
//     })
// })